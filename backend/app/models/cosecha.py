...
class Cosecha(Base):
    ...
    __table_args__ = (
        CheckConstraint('peso_total_kg > 0', name='cosechas_peso_total_kg_check'),
        CheckConstraint('cantidad_peces > 0', name='cosechas_cantidad_peces_check'),
        CheckConstraint('peso_promedio_g IS NULL OR peso_promedio_g >= 0', name='cosechas_peso_promedio_g_check'),
        {"schema": "biofloc"}
    )
    ...
    peso_total_kg: Mapped[float] = mapped_column(Numeric(12, 3), nullable=False)
    peso_promedio_g: Mapped[Optional[float]] = mapped_column(Numeric(10, 3), nullable=True)
