...
def _audit(db, usuario_id, accion, registro_id, detalle: dict):
    ...
    for k, v in detalle.items():
        if isinstance(v, (date, datetime)): safe[k] = v.isoformat()
        else: safe[k] = v
    db.add(Auditoria(usuario_id=usuario_id, tabla="eventos_energia", registro_id=registro_id, accion=accion, detalle=safe))
...
def _duracion_minutos(inicio: datetime, fin: Optional[datetime]) -> Optional[int]:
    ...
    if mins < 0: raise HTTPException(status_code=422, detail="fecha_hora_fin debe ser >= fecha_hora_inicio")
    return mins
...
def listar_eventos_energia(db: Session, tipo: Optional[str] = None, fecha_desde: Optional[datetime] = None, fecha_hasta: Optional[datetime] = None, respaldo_activado: Optional[bool] = None, equipo_respaldo_id: Optional[int] = None, registrado_por: Optional[int] = None):
    ...
    if tipo: q = q.filter(EventoEnergia.tipo.ilike(f"%{tipo}%"))
    if fecha_desde: q = q.filter(EventoEnergia.fecha_hora_inicio >= fecha_desde)
    if fecha_hasta: q = q.filter(EventoEnergia.fecha_hora_inicio <= fecha_hasta)
    if respaldo_activado is not None: q = q.filter(EventoEnergia.respaldo_activado == respaldo_activado)
    if equipo_respaldo_id is not None: q = q.filter(EventoEnergia.equipo_respaldo_id == equipo_respaldo_id)
    if registrado_por is not None: q = q.filter(EventoEnergia.registrado_por == registrado_por)
...
def obtener_evento_energia(db: Session, evento_id: int) -> EventoEnergia:
    ...
    if not e: raise HTTPException(status_code=404, detail="Evento de energía no encontrado")
    return e
...
def crear_evento_energia(db: Session, data: EventoEnergiaCreate, usuario_id: int) -> EventoEnergia:
    ...
    try:
        nuevo = EventoEnergia(fecha_hora_inicio=data.fecha_hora_inicio, fecha_hora_fin=data.fecha_hora_fin, duracion_minutos=duracion, tipo=tipo, respaldo_activado=respaldo, equipo_respaldo_id=data.equipo_respaldo_id, observaciones=data.observaciones.strip() if data.observaciones else None, registrado_por=usuario_id)
        db.add(nuevo); db.flush()
        _audit(db, usuario_id, "INSERT", nuevo.id, {"fecha_hora_inicio": nuevo.fecha_hora_inicio, "fecha_hora_fin": nuevo.fecha_hora_fin, "duracion_minutos": nuevo.duracion_minutos, "tipo": nuevo.tipo, "respaldo_activado": nuevo.respaldo_activado, "equipo_respaldo_id": nuevo.equipo_respaldo_id, "observaciones": nuevo.observaciones}); db.commit()
    except HTTPException:
        db.rollback(); raise
    except IntegrityError:
        db.rollback(); raise HTTPException(status_code=400, detail="No fue posible registrar el evento de energía por una regla de integridad.")
    except Exception:
        db.rollback(); raise HTTPException(status_code=500, detail="No fue posible registrar el evento de energía por un error interno.")
...
def actualizar_evento_energia(db: Session, evento_id: int, data: EventoEnergiaUpdate, usuario_id: int) -> EventoEnergia:
    ...
    try:
        for k, v in cambios.items(): setattr(e, k, v)
        db.flush(); _audit(db, usuario_id, "UPDATE", e.id, cambios); db.commit()
    except HTTPException:
        db.rollback(); raise
    except IntegrityError:
        db.rollback(); raise HTTPException(status_code=400, detail="No fue posible actualizar el evento de energía por una regla de integridad.")
    except Exception:
        db.rollback(); raise HTTPException(status_code=500, detail="No fue posible actualizar el evento de energía por un error interno.")
    db.refresh(e); return e
