...
def _audit(db, usuario_id, accion, registro_id, detalle: dict):
    ...
    db.add(Auditoria(usuario_id=usuario_id, tabla="equipos", registro_id=registro_id, accion=accion, detalle=safe))
...
def listar_equipos(db: Session, solo_activos: bool = False, tipo_equipo_id: Optional[int] = None, estado_id: Optional[int] = None, codigo: Optional[str] = None, nombre: Optional[str] = None):
    q = db.query(Equipo).options(joinedload(Equipo.tipo), joinedload(Equipo.estado))
    if solo_activos: q = q.filter(Equipo.activo == True)  # noqa: E712
    if tipo_equipo_id is not None: q = q.filter(Equipo.tipo_equipo_id == tipo_equipo_id)
    if estado_id is not None: q = q.filter(Equipo.estado_id == estado_id)
    if codigo: q = q.filter(Equipo.codigo.ilike(f"%{codigo}%"))
    if nombre: q = q.filter(Equipo.nombre.ilike(f"%{nombre}%"))
...
def obtener_equipo(db: Session, equipo_id: int) -> Equipo:
    ...
    if not e: raise HTTPException(status_code=404, detail="Equipo no encontrado")
    return e
...
def crear_equipo(db: Session, data: EquipoCreate, usuario_id: int) -> Equipo:
    ...
    if db.query(Equipo).filter(Equipo.codigo == codigo).first(): raise HTTPException(status_code=409, detail=f"Ya existe un equipo con el código '{codigo}'")
    valor = None
    if data.valor_adquisicion is not None:
        valor = Decimal(data.valor_adquisicion).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        if valor < 0: raise HTTPException(status_code=422, detail="valor_adquisicion debe ser >= 0")
    ...
    try:
        ...
        db.rollback(); raise HTTPException(status_code=400, detail="No fue posible registrar el equipo por una regla de integridad.")
    except Exception:
        db.rollback(); raise HTTPException(status_code=500, detail="No fue posible registrar el equipo por un error interno.")
...
def actualizar_equipo(db: Session, equipo_id: int, data: EquipoUpdate, usuario_id: int) -> Equipo:
    ...
    if "nombre" in cambios: cambios["nombre"] = cambios["nombre"].strip()
    if "valor_adquisicion" in cambios and cambios["valor_adquisicion"] is not None:
        cambios["valor_adquisicion"] = Decimal(cambios["valor_adquisicion"]).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        if cambios["valor_adquisicion"] < 0: raise HTTPException(status_code=422, detail="valor_adquisicion debe ser >= 0")
    for campo in ("marca", "modelo", "numero_serie", "ubicacion", "observaciones"):
        if campo in cambios: cambios[campo] = _strip(cambios[campo])
    try:
        ...
        db.rollback(); raise HTTPException(status_code=400, detail="No fue posible actualizar el equipo por una regla de integridad.")
    except Exception:
        db.rollback(); raise HTTPException(status_code=500, detail="No fue posible actualizar el equipo por un error interno.")
