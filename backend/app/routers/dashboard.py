...
from app.services.finanzas_service import calcular_finanzas
...
from app.schemas.finanzas import DashboardFinanzasOut
...
def dashboard_root(fecha_desde: Optional[date] = None, fecha_hasta: Optional[date] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    return _resumen_impl(fecha_desde, fecha_hasta, db, current_user)


@router.get("/resumen", response_model=DashboardResumenOut)
def dashboard_resumen(fecha_desde: Optional[date] = None, fecha_hasta: Optional[date] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    return _resumen_impl(fecha_desde, fecha_hasta, db, current_user)


@router.get("/finanzas", response_model=DashboardFinanzasOut)
def finanzas(fecha_desde: Optional[date] = None, fecha_hasta: Optional[date] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    _auth(db, current_user)
    _validar_periodo(fecha_desde, fecha_hasta)
    return calcular_finanzas(db, fecha_desde, fecha_hasta)


@router.get("/inventario", response_model=DashboardInventarioOut)
def inventario(fecha_desde: Optional[date] = None, fecha_hasta: Optional[date] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    _auth(db, current_user)
    _validar_periodo(fecha_desde, fecha_hasta)
...
def compras(fecha_desde: Optional[date] = None, fecha_hasta: Optional[date] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    _auth(db, current_user)
    _validar_periodo(fecha_desde, fecha_hasta)
...
def ventas(fecha_desde: Optional[date] = None, fecha_hasta: Optional[date] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    _auth(db, current_user)
    _validar_periodo(fecha_desde, fecha_hasta)
...
def gastos(fecha_desde: Optional[date] = None, fecha_hasta: Optional[date] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    _auth(db, current_user)
    _validar_periodo(fecha_desde, fecha_hasta)
...
def equipos(fecha_desde: Optional[date] = None, fecha_hasta: Optional[date] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    _auth(db, current_user)
    _validar_periodo(fecha_desde, fecha_hasta)
...
def energia(fecha_desde: Optional[date] = None, fecha_hasta: Optional[date] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    _auth(db, current_user)
    _validar_periodo(fecha_desde, fecha_hasta)
...
def alarmas(fecha_desde: Optional[date] = None, fecha_hasta: Optional[date] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    _auth(db, current_user)
    _validar_periodo(fecha_desde, fecha_hasta)
...
def produccion(fecha_desde: Optional[date] = None, fecha_hasta: Optional[date] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    _auth(db, current_user)
    _validar_periodo(fecha_desde, fecha_hasta)
