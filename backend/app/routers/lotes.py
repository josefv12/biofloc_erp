...
def listar(estanque_id: Optional[int] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    _require_roles(current_user, db, {"ADMINISTRADOR", "TECNICO", "OPERARIO"})
    return svc.listar_lotes(db, estanque_id)


@router.get("/{lote_id}/costos", response_model=CostosLoteOut, summary="Resumen de costos del lote")
...
def obtener(lote_id: int, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    _require_roles(current_user, db, {"ADMINISTRADOR", "TECNICO", "OPERARIO"})
    return svc.obtener_lote(db, lote_id)


@router.post("/", response_model=LoteOut, status_code=201, summary="Crear lote")
def crear(data: LoteCreate, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
...
def actualizar(lote_id: int, data: LoteUpdate, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    _require_roles(current_user, db, {"ADMINISTRADOR", "TECNICO"})
