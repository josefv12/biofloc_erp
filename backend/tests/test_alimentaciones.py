#!/usr/bin/env python3
"""
Script de pruebas reales para el bloque Alimentaciones — Biofloc ERP V1
Ejecutar: python test_alimentaciones.py

Pruebas:
 1. Login real + JWT
 2. POST alimentación válida
 3. GET alimentación por ID
 4. GET listado
 5. GET listado filtrado por lote_id
 6. Asociación correcta con lote y producto
...
def test_crear_alimentacion(token, lote_id):
    ...
    if ok:
        data = r.json()
        a_id = data["id"]
        TEST_ALIMENTACION_IDS.append(a_id)
        detail = f"id={a_id} lote_id={data['lote_id']} cantidad={data['cantidad']}"
    else:
...
def test_get_by_id(token, a_id):
    ...
    if ok:
        data = r.json()
        detail = f"id={data['id']} lote_id={data['lote_id']} producto_id={data['producto_id']} created_at={data['created_at']}"
    else:
...
def test_listar_filtrado(token, lote_id):
    ...
    ok = r.status_code == 200 and isinstance(r.json(), list)
    if ok:
        todos_del_lote = all(a["lote_id"] == lote_id for a in r.json())
...
def test_asociacion_lote(a_id, lote_id):
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT lote_id, producto_id FROM biofloc.alimentaciones WHERE id = %s;", (a_id,)
        )
        row = cur.fetchone()
        cur.close()
        conn.close()
...
def test_rol_operario(lote_id):
    ...
    if ok:
        a_id = r.json()["id"]
        TEST_ALIMENTACION_IDS.append(a_id)
        detail = f"status=201 id={a_id} (OPERARIO si tiene permiso)"
    else:
        detail = f"status={r.status_code} | {r.text[:80]}"
...
def test_auditoria(a_id):
    try:
        ...
        cur.execute(
            ...
            "WHERE tabla = 'alimentaciones' AND registro_id = %s ORDER BY id DESC LIMIT 1;",
            (a_id,)
        )
        row = cur.fetchone()
        cur.close()
        conn.close()
        ok = row is not None and row[1] == "INSERT" and row[2] == "alimentaciones"
...
def limpiar_datos_prueba():
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "DELETE FROM biofloc.auditoria WHERE tabla = 'alimentaciones' AND registro_id = ANY(%s);",
            (TEST_ALIMENTACION_IDS,)
        )
        cur.execute(
            "DELETE FROM biofloc.alimentaciones WHERE id = ANY(%s);",
            (TEST_ALIMENTACION_IDS,)
        )
        conn.commit()
        cur.close()
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("  BIOFLOC ERP V1 - Pruebas Alimentaciones")
    print("="*60)
    ...
    a_id = test_crear_alimentacion(token, lote_id) # 2
    if a_id:
        test_get_by_id(token, a_id)                # 3
        test_asociacion_lote(a_id, lote_id)        # 6
        test_auditoria(a_id)                       # 11
    test_listar(token)                             # 4
