...
export function ComprasPage() {
  ...
  return (
    <div>
      <PageHeader title="Compras" description="Registrar una compra genera automáticamente la entrada de inventario. No se llama a movimientos desde esta pantalla." actions={puedeRegistrar ? <button type="button" className="bf-btn-primary" onClick={openCreate}>Nueva compra</button> : null} />
      <div className="mb-4 flex flex-wrap items-end gap-3">
        <label className="text-sm"><span className="mb-1 block text-[var(--bf-muted)]">Desde</span><input type="date" className="bf-input" value={fechaDesde} onChange={(e) => setFechaDesde(e.target.value)} /></label>
        <label className="text-sm"><span className="mb-1 block text-[var(--bf-muted)]">Hasta</span><input type="date" className="bf-input" value={fechaHasta} onChange={(e) => setFechaHasta(e.target.value)} /></label>
      </div>
      {comprasQuery.isLoading ? <LoadingState /> : null}
      {comprasQuery.isError ? <ErrorAlert message={apiErrorMessage(comprasQuery.error)} /> : null}
      {comprasQuery.data ? <DataTable rows={comprasQuery.data} rowKey={(row: Compra) => row.id} empty="No hay compras." onRowClick={(row) => navigate(`/compras/${row.id}`)} columns={[
        ...
        { key: "ver", header: "", render: (row) => <Link to={`/compras/${row.id}`} className="text-sm text-[var(--bf-accent)]" onClick={(e) => e.stopPropagation()}>Ver detalle</Link> },
      ]} /> : null}
      <Modal open={open} title="Nueva compra" size="lg" onClose={() => setOpen(false)}>
        <form className="space-y-4" onSubmit={(event) => {
          ...
          <div className="space-y-3">
            ...
            <button type="button" className="bf-btn-secondary" onClick={() => setLineas((rows) => [...rows, { key: crypto.randomUUID(), producto_id: productosQuery.data?.[0] ? String(productosQuery.data[0].id) : "", cantidad: "", precio_unitario: "" }])}>Agregar producto</button>
          </div>
