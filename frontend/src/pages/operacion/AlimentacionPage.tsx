...
import { cantidadDesdePresentacion, cantidadParaPresentacion, unidadPresentacion } from "../../utils/unidades";
...
export function AlimentacionPage() {
  ...
  return (
    <div>
      <PageHeader title="Alimentación" description="Registro de alimento por lote. El consumo actualiza automáticamente el inventario." />
      <label className="mb-4 block max-w-xs text-sm">
        <span className="mb-1 block text-[var(--bf-muted)]">Lote</span>
        <select className="bf-input" value={loteId ?? ""} onChange={(event) => {
          const next = new URLSearchParams(params);
          if (event.target.value) next.set("lote_id", event.target.value); else next.delete("lote_id");
          setParams(next);
        }}>
          <option value="">Todos</option>
          {(lotesQuery.data ?? []).map((lote) => <option key={lote.id} value={lote.id}>{lote.codigo}</option>)}
        </select>
      </label>
...
export function AlimentacionPanel({ loteId, lotes, compact }: { loteId?: number; lotes: Lote[]; compact: boolean }) {
  ...
  const unidadVisible = unidadPresentacion(simboloUnidad);
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (resp) => {
      ...
      if (resp.stock_restante != null) {
        ...
        const restante = cantidadParaPresentacion(resp.stock_restante, simbolo);
  ...
  return (
    <div>
      {stockMsg ? <div className="mb-3 rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-sm text-green-800">{stockMsg}</div> : null}
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-[var(--bf-muted)]">{compact ? "Últimas alimentaciones de este lote" : "Historial de alimentación"}</p>
        <div className="flex gap-2">
          {compact && loteId ? <Link to={`/operacion/alimentacion?lote_id=${loteId}`} className="bf-btn-secondary">Ver todas</Link> : null}
          {can(user?.rol, "registrarAlimentacion") && puedeRegistrarLote ? (
            <button type="button" className="bf-btn-primary" onClick={() => {
              setFormError(null);
              form.reset({ lote_id: loteId ?? enProduccion[0]?.id ?? 0, producto_id: productosQuery.data?.[0]?.id ?? 0, fecha_hora: toDatetimeLocalValue(), cantidad: "", observaciones: "" });
              setOpen(true);
            }}>Registrar alimentación</button>
          ) : null}
        </div>
      </div>
      ...
      {query.data ? (
        <DataTable rows={rows} rowKey={(row: Alimentacion) => row.id} empty="No hay alimentaciones." columns={[
          ...
          { key: "cantidad", header: "Cantidad", render: (row) => { const producto = productos.get(row.producto_id); const simbolo = producto ? unidades.get(producto.unidad_id)?.simbolo : undefined; return `${formatNumber(cantidadParaPresentacion(row.cantidad, simbolo), { maximumFractionDigits: 3 })}${simbolo ? ` ${unidadPresentacion(simbolo)}` : ""}`; } },
      ...
      <Modal open={open} title="Registrar alimentación" onClose={() => setOpen(false)}>
        <form className="space-y-3" onSubmit={form.handleSubmit((values) => {
          ...
          const simbolo = producto ? unidades.get(producto.unidad_id)?.simbolo : undefined;
          const cantidad = cantidadDesdePresentacion(Number(values.cantidad), simbolo);
          ...
          {contextoQuery.isLoading ? <p className="text-sm text-[var(--bf-muted)]">Calculando ración recomendada…</p> : ref ? <ContextoAlimentacionPanel ref={ref} /> : contextoQuery.isSuccess ? <p className="text-sm text-[var(--bf-muted)]">N/D — Sin referencia configurada.</p> : null}
          {loteId ? <input type="hidden" {...form.register("lote_id", { valueAsNumber: true })} /> : (
            <Field label="Lote"><select className="bf-input" {...form.register("lote_id", { valueAsNumber: true })}>{enProduccion.map((row) => <option key={row.id} value={row.id}>{row.codigo}</option>)}</select></Field>
          )}
          ...
          <Field label="Fecha y hora"><input type="datetime-local" className="bf-input" {...form.register("fecha_hora", { required: true })} /></Field>
          <Field label={etiquetaCantidad}><input type="number" step="any" min="0.0001" className="bf-input" {...form.register("cantidad", { required: true })} /></Field>
          <Field label="Observaciones"><textarea className="bf-input min-h-20" {...form.register("observaciones")} /></Field>
          <p className="text-xs text-[var(--bf-muted)]">La cantidad se captura en la unidad comercial mostrada y se convierte automáticamente a la unidad interna antes de enviarla al servidor.</p>
