...
import { etiquetaProducto, formatDateTime, formatNumber, toDatetimeLocalValue, withFechaHoraIso } from "../../utils/format";
...
export function AlimentacionPage() {
  ...
  return <div><PageHeader title="Alimentación" description="Registro de alimento por lote. El consumo actualiza automáticamente el inventario." /><label className="mb-4 block max-w-xs text-sm"><span className="mb-1 block text-[var(--bf-muted)]">Lote</span><select className="bf-input" value={loteId ?? ""} onChange={(event) => { const next = new URLSearchParams(params); if (event.target.value) next.set("lote_id", event.target.value); else next.delete("lote_id"); setParams(next); }}><option value="">Todos</option>{(lotesQuery.data ?? []).map((lote) => <option key={lote.id} value={lote.id}>{lote.codigo}</option>)}</select></label><AlimentacionPanel loteId={loteId} lotes={lotesQuery.data ?? []} compact={false} /></div>;
}
...
export function AlimentacionPanel({ loteId, lotes, compact }: { loteId?: number; lotes: Lote[]; compact: boolean }) {
  ...
  return <div>
    ...
    <div className="mb-3 flex flex-wrap items-center justify-between gap-2"><p className="text-sm text-[var(--bf-muted)]">{compact ? "Últimas alimentaciones de este lote" : "Historial de alimentación"}</p><div className="flex gap-2">{compact && loteId ? <Link to={`/operacion/alimentacion?lote_id=${loteId}`} className="bf-btn-secondary">Ver todas</Link> : null}{can(user?.rol, "registrarAlimentacion") && puedeRegistrarLote ? <button type="button" className="bf-btn-primary" onClick={() => { setFormError(null); form.reset({ lote_id: loteId ?? enProduccion[0]?.id ?? 0, producto_id: productosQuery.data?.[0]?.id ?? 0, fecha_hora: toDatetimeLocalValue(), cantidad: "", observaciones: "" }); setOpen(true); }}>Registrar alimentación</button> : null}</div></div>
    {query.isLoading ? <LoadingState /> : null}{query.isError ? <ErrorAlert message={apiErrorMessage(query.error)} /> : null}
    {query.data ? <DataTable rows={rows} rowKey={(row: Alimentacion) => row.id} empty="No hay alimentaciones." columns={[
